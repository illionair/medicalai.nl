...
## Corpus Check
- 118 files · ~217,427 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 335 nodes · 478 edges · 11 communities detected
...
5. `getBlogsForTopic()` - 9 edges
6. `hasPostgresDatabaseConfig()` - 9 edges
7. `getCurrentUser()` - 9 edges
8. `escapeXml()` - 9 edges
9. `wrapWords()` - 9 edges
10. `requestMagicLink()` - 8 edges
...
Cohesion: 0.12
Nodes (27): Home(), TopicPage(), getBlogById(), getBlogsForTopic(), getCategoryCounts(), getPublishedBlogs(), getPublishedBlogsByCategory(), getPublishedBlogsByTag() (+19 more)
...
- **Why does `requireAdmin()` connect `Community 0` to `Community 2`, `Community 7`?**
  _High betweenness centrality (0.123) - this node is a cross-community bridge._
- **Why does `getStaticArticleById()` connect `Community 1` to `Community 0`?**
  _High betweenness centrality (0.099) - this node is a cross-community bridge._
- **Why does `getCover()` connect `Community 3` to `Community 1`, `Community 11`?**
  _High betweenness centrality (0.099) - this node is a cross-community bridge._
...
- **Are the 4 inferred relationships involving `getBlogsForTopic()` (e.g. with `TopicPage()` and `getStaticArticles()`) actually correct?**
  _`getBlogsForTopic()` has 4 INFERRED edges - model-reasoned connections that need verification._